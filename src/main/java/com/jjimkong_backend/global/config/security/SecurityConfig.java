    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests(authorize -> authorize
                        ...
                        // 평가 기준 추가/수정/삭제는 ADMIN 전용 (조회는 인증 사용자면 가능)
                        .requestMatchers(HttpMethod.POST, "/api/v1/review-criteria").hasRole("ADMIN")
                        .requestMatchers(HttpMethod.PUT, "/api/v1/review-criteria/**").hasRole("ADMIN")
                        .requestMatchers(HttpMethod.DELETE, "/api/v1/review-criteria/**").hasRole("ADMIN")
